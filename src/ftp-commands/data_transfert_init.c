...
static void init_data_socket(socket_t *data_sock, int port)
{
...
void port_f(ftp_t *ftp, char *arg, client_list_t *client)
{
    ...
    if (index != 6) {
        ...
        if (!client->data_sock)
            return;
        init_data_socket(client->data_sock, int_tab[4] * 256 + int_tab[5]);
...
void pasv_f(ftp_t *ftp, char *arg, client_list_t *client)
{
    ...
    if (!client->data_sock)
        return;
    init_data_socket(client->data_sock, 0);
    dprintf(client->fd, "227 Entering Passive Mode %s.\n", \
    disp_port(&client->data_sock->addr, client->data_sock->addr.sin_addr));
